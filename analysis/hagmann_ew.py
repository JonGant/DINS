            ...
            if adj_mat[node1, node2] == 0 or adj_mat[node2, node1] == 0:
                ...
                for streamline in gen:
                    ...
                    for i in range(len(streamline) - 1):
                        length += res * np.linalg.norm(streamline[i + 1] - streamline[i])
                    sum += (1 / length)
                # edge weight calculation
                adj_mat[node1, node2] = (2 / (sa[node1] + sa[node2])) * sum
                adj_mat[node2, node1] = adj_mat[node1, node2]
        np.savetxt(fname[22:-4] + "_" + str(s) + "_cubic_hagmann_ew_wm.txt", adj_mat, delimiter=" ")